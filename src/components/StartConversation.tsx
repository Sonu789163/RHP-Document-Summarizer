...
export const StartConversation: React.FC = () => {
  ...
  const [movingDocDirectoryId, setMovingDocDirectoryId] = useState<string | null>(null);
      ...
      (async () => {
        try {
          ...
          if (link?.resourceType === 'document') {
            const doc = await documentService.getById(link.resourceId, persistedToken);
  ...
  const handleUpload = async (file: File) => {
    ...
    // Fast client-side duplicate guard by namespace
    const localNamespace = uploadService.normalizeNamespace(file.name).toLowerCase();
    const localDup = (documents || []).find((d: any) =>
      (uploadService
        .normalizeNamespace(d?.namespace)
        .toLowerCase()) === localNamespace
    );
    if (localDup) {
    ...
    try {
      ...
      if (existing.exists && existing.document) {
        ...
        toast(
          <div className="flex items-center gap-2">
            <AlertCircle className="h-4 w-4 text-destructive" />
            <span>
              Document "{existing.document.name}" already exists and is highlighted below.
            </span>
          </div>,
          { duration: 4000 }
        );
      ...
      formData.append("namespace", uploadService.normalizeNamespace(file.name));
        ...
        if (res.status === 409) {
          ...
          toast(
            <div className="flex items-center gap-2">
              <AlertCircle className="h-4 w-4 text-destructive" />
              <span>
                Document "{response?.existingDocument?.name}" already exists and is highlighted below.
              </span>
            </div>,
            { duration: 4000 }
          );
  ...
  const filteredDocs = documents.filter((doc) => {
    ...
    if (timeFilter !== 'all') {
      ...
      // Debug logging
      console.log(`Filter: ${timeFilter}, Filter Start: ${filterStart.toLocaleDateString()}, Doc Date: ${docDate.toLocaleDateString()}, Doc Name: ${doc.name}, Pass: ${docDate >= filterStart}`);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (showTimeFilter) {
        const timeFilterButton = document.querySelector('[data-time-filter="true"]');
        if (timeFilterButton && !timeFilterButton.contains(event.target as Node)) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-row min-w-0">
        ...
        <FolderSidebar
          onFolderOpen={(folder) => setCurrentFolder({ id: folder.id, name: folder.name })}
          onFolderDeleted={() => {
            setCurrentFolder(null);
            fetchDocuments();
          }}
          ...
          <div className="flex  items-center">
                ...
                {isDateRangeApplied ? (
                  <>
                    {startDate && endDate ? `${startDate} - ${endDate}` : 
                     ...
                     endDate ? `Until ${endDate}` : 'Date Range'}
                    <X 
            ...
            {currentFolder?.name && (
                ...
                >
                  ...
                  <span className="truncate max-w-[18vw]" title={currentFolder.name}>{currentFolder.name}</span>
                </div>
                </button>
            )}
          </div>
